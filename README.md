...
<br/>Запустив этот код командой: `./qvm -i file.qasm -b 16`, (я на линуксе, на винде вроде будет `.\qvm -i file.qasm -b 16`), виртуальная машина запишет в свою память вот такой байткод:
```bash
...
<br/>Такая реализация кубита соответсвует формуле `Ψ = α|0⟩ + β|1⟩`, так что, мое решение можно назвать одним из верных. Я на это надеюсь. *~~(Хотя спустя полгода, теперь я пишу эту доку и мне кажется, что где-то тут, что-то не так.)~~* 
<br/>

<a id="qubits2"></a>
#### Реализованные операции
Все операции с кубитами прописаны как методы квантового регистра (за исключением [операции **RST**](#qop_rst)).
<br/>Мне кажется, будет проще сделать вот такую таблицу:
<table>
  <thead>
    <tr>
      <th>Операция</th>
  ...
  <tbody>
    <tr>
      <td>meas</td>
      <td>Измеряет значение кубита, тем самым выводит его из состояния суперпозиции</td>
    </tr>
    <tr>
      <td>H</td>
      <td>Вводит кубит в состояние суперпозиции</td>
    </tr>
    <tr>
      <td>X <i>(NOT)</i></td>
      <td>Меняет местами α и β кубита</td>
    </tr>
    <tr>
      <td>Y</td>
      <td>// TODO:</td>
...
<a id="dbg"></a>
## Дебаг версия
В репозитории есть Makefile, если просто прописать команду `make`, то скомпилируется обычная версия, но для удобства разработки и тестов, есть дебаг версия, ее можно получить прописав `make dbg`.
<br/>Дебаг версия выводит сообщение о каждой инструкции, которую исполняет - это помогает понять, как виртуальная машина работает.
<br/>
