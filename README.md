...
<br/>`00` и `00` - эти 2 байта являются **кодом метки** `_test` *(они оба нулевые, т.к. первый код - 0x0000, если в коде появится вторая метка, у нее будет код 0x0001 и т.д.)*
...
```
`48` - опкод [инструкции **jmp**](#qasm_cinstrs_jmp)
<br/>`00` и `00` - эти 2 байта являются **кодом метки** `_lbl` *(они оба нулевые, т.к. первый код - 0x0000, если в коде появится вторая метка, у нее будет код 0x0001 и т.д.)*
<br/>`01` - этот байт говорит о том, что переменная `a` занимает 1 байт *(адрес переменной ссылается именно на этот байт)*
<br/>`0d` - этот байт обозначает литерал 13
<br/>`12` - опкод [инструкции **mov**](#qasm_cinstrs_mov)
<br/>`1a` - опкод [классического регистра](#cregs) **al**
<br/>`00` и `00` - эти 2 байта являются **кодом переменной** `a` *(они оба нулевые, т.к. первый код - 0x0000, если в коде появится вторая переменная, у нее будет код 0x0001 и т.д.)*
<br/>`ff` - опкод [инструкции **hlt**](#qasm_cinstrs_hlt)
